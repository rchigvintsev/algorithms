...
public class SimpleBinarySearchTreeTest {
    private SimpleBinarySearchTree<String, Integer> bst;
    ...
    public void testLevelOrder() {
        testIterable(bst.levelOrder());
        bst.put("2", 2);
        bst.put("3", 3);
        bst.put("4", 4);
        testIterable(bst.levelOrder(), new String[] {"2", "3", "4"});
        bst.put("0", 0);
        testIterable(bst.levelOrder(), new String[] {"2", "0", "3", "4"});
        bst.put("1", 1);
        testIterable(bst.levelOrder(), new String[] {"2", "0", "3", "1", "4"});
        bst.remove("2");
        testIterable(bst.levelOrder(), new String[] {"3", "0", "4", "1"});
    }
    ...
    public void testOrderSymmetry() {
        ...
        for (int i = 0; i < letters.length; i++)
            bst.put(letters[i], i);
        testOrderSymmetry(bst.root, null, null);
    }
    ...
    private void testSizeConsistency(SimpleBinarySearchTree<String, Integer>.Node node) {
        if (node == null)
            return;
        assertFalse(node.size != bst.size(node.left) + bst.size(node.right) + 1);
        testSizeConsistency(node.left);
        testSizeConsistency(node.right);
    }

    private <T> void testIterable(Iterable<T> iterable) {
        testIterable(iterable, null);
    }

    private <T> void testIterable(Iterable<T> iterable, T[] expectedValues) {
