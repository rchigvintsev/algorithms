 ...
 */
public class OptimizedLongQuickSortAlgorithm implements LongSortAlgorithm {
    ...
    private int median3(long[] a, int i, int j, int k) {
        return a[i] < a[j]
                ? (a[j] < a[k] ? j : (a[i] < a[k] ? k : i))
                : (a[k] < a[j] ? j : (a[k] < a[i] ? k : i));
    }

    private int tukeyNinther(long[] a, int lo, int mid, int hi) {
        int length = hi - lo + 1;
        int eps = length / 8;
